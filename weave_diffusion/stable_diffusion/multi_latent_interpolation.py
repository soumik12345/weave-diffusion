...
class StableDiffusionMultiLatentInterpolationPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
    ) -> List[StableDiffusionInterpolationOutput]:
        height = height or self.unet.config.sample_size * self.vae_scale_factor
        width = width or self.unet.config.sample_size * self.vae_scale_factor
        channels = self.unet.config.in_channels

        latents = torch.randn(
            (2, channels, height // 8, width // 8), generator=generator
        )
